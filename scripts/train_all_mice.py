...
class SpikeTransformerHierarchical(nn.Module):
    ...
    def forward(self, waveforms, shank_ids, mask):
        ...
        sigmas = [torch.exp(head(pooled).clamp(-7, 2)) for head in self.log_sigma_heads]
    ...
    for batch in loader:
        ...
        for z in range(N_ZONES):
            ...
            if zmask.any():
                ...
                n_active_zones += 1
        if n_active_zones > 0:
            loss_pos = loss_pos / n_active_zones
...
# ============================================================
def _validate_json_for_parquet(json_path, parquet_path):
    """Vérifie que le JSON est compatible avec le parquet en testant un reshape."""
    try:
        with open(json_path, 'r') as f:
            params = json.load(f)
        nGroups = params['nGroups']
        nChannelsPerGroup = [params[f'group{g}']['nChannels'] for g in range(nGroups)]
        df = pd.read_parquet(parquet_path, columns=[f'group{g}' for g in range(nGroups)])
        row = df.iloc[0]
        for g in range(nGroups):
            raw = row[f'group{g}']
            raw.reshape(-1, nChannelsPerGroup[g], 32)
        return True
    except Exception:
        return False
...
def discover_datasets(data_dir):
    ...
    for pq in parquets:
        ...
        # D'abord essayer par nom (region dans le nom du json)
        best_json = None
        ...
        # Si pas de match par nom, valider chaque JSON en testant le reshape
        if best_json is None:
            for j in matching_jsons:
                if _validate_json_for_parquet(j, pq):
                    best_json = j
                    break

        if best_json is None:
            print(f'  [WARN] Aucun JSON compatible trouvé pour {pq_name}, skip')
            continue
...
def train_mouse(dataset_info):
    ...
    output_dir = os.path.join(OUTPUT_BASE, f'{mouse_id}_{region}')
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(df_train_full)):
        ...
        fold_results.append({
            'fold': fold + 1, 'best_val_loss': float(best_val_loss),
            'val_eucl_mean': float(val_eucl.mean()),
            'val_r2_x': float(r2_score(val_targets[:, 0], val_mu[:, 0])),
            'val_r2_y': float(r2_score(val_targets[:, 1], val_mu[:, 1])),
            'val_cls_acc': float(val_acc),
        })
...
def main():
    ...
    print(f'{"="*60}')
    for r in results:
        print(f'  {r["mouse_id"]}_{r["region"]} : Eucl={r["eucl_mean"]:.4f} | MSE: X={r["mse_x"]:.5f} Y={r["mse_y"]:.5f} | R2: X={r["r2_x"]:.4f} Y={r["r2_y"]:.4f} | cls={r["cls_accuracy"]:.1%}')
